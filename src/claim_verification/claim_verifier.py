...
Process each claim one by one with these calibration rules in mind.
        """.strip()
    ...
    def verify_claims_with_improvements(self, claims, quality_report: QualityReport | None = None):
        ...
        Args:
            claims (_type_): The claims extracted
            quality_report (QualityReport): The quality report generated by the quality checker
        """
        if not self.is_setup:
            print(f"Please setup the Claim Verifier first")
            return None, None
